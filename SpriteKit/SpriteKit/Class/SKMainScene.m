...
- (instancetype)initWithSize:(CGSize)size{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(nomusicplayer:) name:@"nomusicplayer" object:nil];
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(moshi:) name:@"moshi" object:nil];
    }
    return self;
}

-(void)nomusicplayer:(NSNotification*) notification{
...
-(void)moshi:(NSNotification*) notification{
    if ([[notification object] isEqualToString:@"0"]) {
        _jindianmoshi = NO;
    }else{
        _jindianmoshi = YES;
...
- (void)initPlayerPlane{
    ...
    _playerPlane.position = CGPointMake(160, 50);//悟空初始位置
    _playerPlane.zPosition = 1;
    CGSize uncheckedsize = CGSizeMake(10.0f, 10.0f);//悟空受伤范围
    _playerPlane.physicsBody = [SKPhysicsBody bodyWithRectangleOfSize:uncheckedsize];//悟空
...
- (void)createFoePlane{
    ...
    //原值为25
    int smallplaneTimeNumer= 0;
    if (_jindianmoshi) {
        smallplaneTimeNumer = 10;
    }else{
        smallplaneTimeNumer = 20;
    }
    if (_smallPlaneTime > smallplaneTimeNumer)
    {
    ...
    if (_musicplayer) {
        [sprite runAction:[SKSharedAtles playerPlaneBlowupAction] completion:^{
            [self runAction:[SKAction sequence:@[[SKAction playSoundFileNamed:@"game_over.mp3" waitForCompletion:YES],[SKAction runBlock:^{
                ...
                label.text = _scoreLabel.text;//@"GameOver";
                label.zPosition = 2;
                ...
                [[NSNotificationCenter defaultCenter]postNotificationName:@"gameOverNotification" object:_scoreLabel.text];
            }];
        }];
    }else{
        [sprite runAction:[SKSharedAtles playerPlaneBlowupAction] completion:^{
